...
const Brands = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert("✅ تم إضافة البراند بنجاح");
      window.location = "/control";
    } catch (err) {
      console.error("❌ خطأ في الإرسال:", err);
      alert("حدث خطأ أثناء اضافة البراند");
    }finally{
      setLoading(false)
    }
  };
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      await axios.patch(`${baseUrl}/brands/${editProduct.id}`, payload, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
  ...
  return (
    <div>
      ...
      <h2 className="text-2xl font-bold text-[#c10007] mb-4 text-center">إضافة براند جديد</h2>
      ...
      {/* ✅ جدول المنتجات */}
      <h3 className="text-xl font-semibold mb-2">البراندات</h3>
      <div className="overflow-x-auto">
        <table className="min-w-full border border-gray-200 text-sm">
          ...
          <tbody>
            {categories.length === 0 ? (
              <tr>
                <td colSpan="6" className="text-center p-4 text-red-500">لا يوجد براندات بعد</td>
              </tr>
            ) : (
              categories.map((p, i) => (
                <tr key={p.id}>
